...
function validateRequestBody(body: unknown): body is SignPdfRequestBody {
  ...
  if (typeof b.passphrase !== "string" || b.passphrase.trim() === "")
    return false;
...
export async function POST(request: NextRequest) {
  if (!BSSN_API_URL || !BSSN_API_TOKEN) {
    console.error(
      "[BSSN PROD] Missing environment variables: BSSN_API_URL or BSSN_API_TOKEN",
    );
  ...
  if (!validateRequestBody(body)) {
    return NextResponse.json(
      {
        error:
          "Request tidak valid. Field nik, passphrase, dan file wajib diisi.",
      },
      { status: 400 },
    );
  }
  ...
  if (!response.ok) {
    ...
    // Coba parse JSON dari BSSN untuk meneruskan pesan error yang spesifik
    // (misalnya: passphrase salah, NIK tidak terdaftar, dll)
    let bssnError: Record<string, unknown> = {};
    try {
      bssnError = JSON.parse(responseText);
    } catch {
      // Jika bukan JSON, teruskan sebagai string
      bssnError = { error: responseText || "Permintaan ke BSSN gagal" };
    }

    return NextResponse.json(
      {
        ...bssnError,
        // Pastikan field 'error' selalu ada sebagai fallback
        error:
          bssnError.error ?? bssnError.message ?? "Permintaan ke BSSN gagal",
      },
